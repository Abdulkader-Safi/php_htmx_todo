...
$db = new PDO("mysql:host=localhost;dbname=htmx_todo", "root", "");
...
?>
  ...
  ?>
    <div class="todo-list" hx-get="/api.php?action=todo_list&category=<?= $category_encoded ?>" hx-swap="outerHTML" hx-trigger="todo_added<?= $category_id; ?> from:body">
      <?php
  ...
  case "category_list":
    ...
    ?>
      <a href="#" hx-get="/api.php?action=todo_list&category=<?= urlencode($category); ?>" hx-target=".todo-list" hx-swap="outerHTML" class="category">
        <?= htmlspecialchars($category); ?> <span class="count" hx-get="/api.php?action=category_incomplete_count&category=<?= urlencode($category); ?>" hx-trigger="load, done_count_<?= md5($category); ?> from:body" hx-swap="innerHTML" hx-target="this"></span>
      </a>
    <?php
    }
    break;
